...
public class EpEditor {
    ...
    public static void scaleVideo(EpVideo epVideo, OutputOption outputOption, OnEditorListener onEditorListener) {
        ...
        if (epVideo.getFilters() != null) {
            filter_complex.append(scaleAndCrop(epVideo, outputOption));
        } else {
            cmd.append("-filter_complex");
            filter_complex.append(scaleAndCrop(epVideo, outputOption));
            isFilter = true;
    ...
    public static void merge(List<EpVideo> epVideos, OutputOption outputOption, OnEditorListener onEditorListener) {
        ...
        if (epVideos.size() > 1) {
            ...
            for (int i = 0; i < epVideos.size(); i++) {
                ...
                filter_complex.append("[").append(i).append(":v]").append(filter).append(scaleAndCrop(epVideos.get(i), outputOption))
                        .append("[outv").append(i).append("];");
            }
...
//				.append("-b").append("10M");
    ...
    private static String scaleAndCrop(EpVideo epVideo, OutputOption outputOption) {
